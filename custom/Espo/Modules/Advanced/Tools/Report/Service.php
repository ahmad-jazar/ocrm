...
class Service
{
    ...
    private function fetchReportForRun(string $id, ?User $user = null): Report
    {
        ...
        if (!$this->aclManager->checkEntity($user, $report)) {
    ...
    public function executeGridReport(
        ...
        foreach ($data->getColumns() as $item) {
            if ($this->gridHelper->isColumnNumeric($item, $data->getEntityType())) {
                $numericColumnList[] = $item;
            }
        }

        foreach ($data->getColumns() as $item) {
            if ($this->gridHelper->isColumnSummary($item)) {
                $summaryColumnList[] = $item;
            }

            if ($this->gridHelper->isColumnSubList($item, $data->getGroupBy()[0] ?? null)) {
        ...
        $this->gridResultHelper->fixRows($rows);
