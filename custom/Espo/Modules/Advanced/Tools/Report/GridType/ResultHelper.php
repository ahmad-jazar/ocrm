...
class ResultHelper
{
     ...
     */
    public function fixRows(array &$rows): void
    {
    ...
    public function populateColumnInfo(
        ...
        foreach ($data->getColumns() as $item) {
            ...
            $columnNameMap[$item] = $this->util->translateColumnName($data->getEntityType(), $item);
        }
    }
    ...
    public function calculateSums(Data $data, Result $result): void
    {
        ...
        foreach ($result->getNonSummaryColumnList() ?? [] as $column) {
            $group = $result->getNonSummaryColumnGroupMap()->$column;
