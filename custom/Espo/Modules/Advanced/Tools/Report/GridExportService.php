...
class GridExportService
{
    ...
    public function buildXlsxContents(string $id, ?WhereItem $where, ?User $user = null): string
    {
        ...
        if ($groupCount === 2) {
            foreach ($columnList as $column) {
                $columnLabels[$column] = $this->gridUtil->translateColumnName($entityType, $column);
            }
        }
        ...
        if ($groupCount) {
            ...
            $exportParams['groupLabel'] = $this->gridUtil->translateColumnName($entityType, $group);
        } else {
    ...
    public function getCellDisplayValueFromResult(
        ...
        if ($this->gridHelper->isColumnNumeric($column, $reportResult->getEntityType())) {
            $value = 0;
        }
        ...
        if (
            !$this->gridHelper->isColumnNumeric($column, $reportResult->getEntityType()) &&
            !is_null($value)
        ) {
