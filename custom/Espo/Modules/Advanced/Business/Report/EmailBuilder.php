...
class EmailBuilder
{
            ...
            if (strpos($column , ':')) {
                ...
                if ($function != 'COUNT') {
                    $columnType = $this->metadata
                        ->get(['entityDefs', $report->get('entityType'), 'fields', $field, 'type']);
                }
            }

            if ($columnType === 'float') {
                $view = $this->metadata
                    ->get(['entityDefs', $report->get('entityType'), 'fields', $field ?? $column, 'view']);
    ...
    private function formatCurrency($value, ?string $currency = null): string
    {
        if ($value === "") {
            return $value;
        }
        ...
        if ($currencyFormat) {
            $pad = (int) $this->config->get('currencyDecimalPlaces');
